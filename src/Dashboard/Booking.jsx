        ...
        <div className='mt-16 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-2 '>
            {services.length === 0 ? (
                <div className='flex justify-center items-center h-screen'>
                    <Link to={'/service'}>
                        <img className='ml-[350px] animate-pulse' src="https://i.ibb.co/nnxyZdN/image-removebg-preview-90.png" alt="No services" />
                    </Link>
                </div>
            ) : (
                    ...
                    <div key={index} className=''>
                        <div className="bg-white shadow-xl h-[260px] rounded-xl px-5 space-y-3">
                            <div className="flex justify-between mt-2 relative">
                                ...
                                <div className="mt-4">
                                    <p className={`bg-pink-100 font-bold text-pink-400 px-6 py-2 rounded-md ${item.status === 'Pending' ? 'bg-yellow-100 text-yellow-500' : item.status === 'Done' ? 'bg-green-100 text-green-500' : 'bg-blue-100 text-blue-500'}`}>
                                        {item.status}
                                    </p>
                                </div>
                            </div>
